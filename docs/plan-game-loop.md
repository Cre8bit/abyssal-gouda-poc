...
- ✅ **T0.4 Bathyscaphe + sas** : `src/bathyscaphe.js` — cloche `tinBell.glb` toonifiée, écoutille tournée vers le gouda ; les plongeurs se réveillent *dedans* (spawn à hauteur d'yeux de l'écoutille, regard vers la sortie). Lumières ancrées sur les fixtures du mesh : lampe de cabine sous la coupole + balises sur le hublot au-dessus de la porte et sur le gros hublot sous le seuil (pulsation lente). Collision joueur–cloche analytique (`collideBathyscaphe` : parois annulaires + plancher + coupole, écoutille ouverte), appliquée après la collision fromage dans `main.js`. Une cloche amarrée par plongeur (`setBellCount`, appelé à chaque join/leave), re-berthées au rebuild. Banc (`preview.js`, id `bell`) + shot `bell` (`&bells=N` pour la rangée). Le sas reste logique (zone `RECHARGE_RADIUS` de `main.js`), la victoire viendra avec T1.3.
